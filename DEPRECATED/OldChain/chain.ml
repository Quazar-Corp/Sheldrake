...
let length chain = List.length chain
...
let add_block chain block =
  ...
  Block.update_index block idx |> fun () ->
  Block.update_hash block (hash_of_string (Block.to_string block)) |> fun () ->
  block :: chain
...
let is_valid chain =
  let rec aux = function
    ...
    | prev :: (curr :: _ as tl) ->
        if Block.valid_crypto prev curr && Block.verify_merkle_root prev then
          aux tl
        else false
  in
  aux chain
...
let mine_block chain transactions =
  ...
  if List.length chain = 0 then
    Block.create ~nonce:0 ~transactions:[]
      ~prev_hash:(String.init 64 (fun _ -> '0'))
  else
    let nonce =
      proof_of_work (Block.get_nonce (get_previous_block chain))
      (* last block nonce *)
    in
    let prev_hash =
      Block.get_hash (get_previous_block chain)
      (* last block hash *)
    in
